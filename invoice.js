...
(function () {
  ...
  function buildReceiptInner(order) {
    ...
    const deliveries = Array.isArray(order?.deliveries) && order.deliveries.length > 1 ? order.deliveries : [];
    const deliveriesHtml = deliveries.length ? `
      <div style="margin:12px 0;padding:11px;border:1px solid #e6ddff;background:#faf8ff;border-radius:12px;text-align:right;">
        <div style="font-weight:900;color:#5b4bc4;margin-bottom:7px;">🧩 تقسيم التسليم</div>
        ${deliveries.map((slot, idx) => {
          const slotItems = Array.isArray(slot?.items) ? slot.items : [];
          const names = slotItems.map(i => `${esc(i?.customName || i?.name || "منتج")} × ${Math.max(0, Number(i?.qty ?? i?.quantity ?? 0))}`).join("، ");
          const mandoub = slot?.mandoub || slot?.deliveryPerson || "بانتظار تحديد المندوب";
          const fee = Math.max(0, Number(slot?.deliveryFee || 0));
          return `<div style="padding:7px 0;${idx ? 'border-top:1px dashed #ddd3ff;' : ''}">
            <div style="font-weight:800;color:#3e355f;">${idx + 1}) ${esc(formatDeliveryDate(slot?.deliveryDate))}</div>
            <div style="font-size:12px;color:#555;margin-top:3px;">🚚 ${esc(mandoub)}${fee > 0 ? ` — ${money(fee)} ر.س` : ""}</div>
            <div style="font-size:12px;color:#555;margin-top:2px;">${names || "—"}</div>
          </div>`;
        }).join("")}
      </div>` : "";
    ...
    const rows = items.map((item) => {
      ...
      const lineAfter = lineBefore * (1 - individual / 100);
      const notes = String(item?.notes || "").trim();
      const individualDiscountHtml = individual > 0
        ? `<tr><td colspan="3" style="text-align:right;font-size:11px;color:#a44848;padding:0 0 5px;font-weight:700;">🏷️ خصم فردي ${money(individual)}% — بعد الخصم ${money(lineAfter)} ر.س</td></tr>`
        : "";
      return `
        <tr>
          ...
          <td style="text-align:left;padding:4px 0;vertical-align:top;white-space:nowrap;">${money(lineBefore)} ر.س</td>
        </tr>
        ${individualDiscountHtml}
