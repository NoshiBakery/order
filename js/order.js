/* =========================================================
   ...
   User-facing times must always use the 12-hour clock (ص / م).
   Keep stored date/time values unchanged; format only when displaying.
========================================================= */
/* =========================================================
   FILE: js/order.js
   SOURCE: inline script #1 from order.html
...
// إعدادات حجم الخط
  ...
  // بيانات التطبيق المشتركة من التخزين السحابي
  let salesData = {};
  let clients = [];
  let backupBaseData = {};
  ...
  // تحميل بيانات الصفحة من التخزين السحابي
  ...
  // حفظ بيانات الطلبات في التخزين السحابي
...
function createBackupFile(label, baseData = backupBaseData) {
  ...
  // نضمن أن النسخة تستخدم آخر بيانات موجودة في الذاكرة بعد التحميل
...
function downloadBackupFiles(beforeFile, afterFile) {
  ...
  // حفظ حجم الخط كإعداد محلي للجهاز
...
function getPhraseStripInfo(order) {
  // شريط العبارة مستثنى من الخصم العام؛ يبقى الخصم فقط.
...
html += `
  ...
  <section class="order-sales-summary order-sales-summary-month">
    <h3 class="order-sales-summary-title">ملخص مبيعات الشهر</h3>

    <div class="order-sales-summary-grid">
      <div class="order-sales-summary-card tone-orange">
        <div class="order-sales-summary-label">عدد الطلبات</div>
        <div class="order-sales-summary-value">${totalOrders}</div>
      </div>

      <div class="order-sales-summary-card tone-green">
        <div class="order-sales-summary-label">إجمالي المبيعات</div>
        <div class="order-sales-summary-value">${grossSales.toFixed(2)} ريال</div>
      </div>

      <div class="order-sales-summary-card tone-orange">
        <div class="order-sales-summary-label">صافي المبيعات</div>
        <div class="order-sales-summary-value">${netSales.toFixed(2)} ريال</div>
      </div>

      <div class="order-sales-summary-card tone-blue">
        <div class="order-sales-summary-label">تكاليف التوصيل</div>
        <div class="order-sales-summary-value">${deliveryCost.toFixed(2)} ريال</div>
      </div>

      <div class="order-sales-summary-card tone-red">
        <div class="order-sales-summary-label">المبالغ المحولة والغير محولة</div>
        <div class="order-sales-summary-value">${unpaidAmount.toFixed(2)} ريال</div>
      </div>

      <div class="order-sales-summary-card tone-purple">
        <div class="order-sales-summary-label">المبالغ المحولة من العملاء</div>
        <div class="order-sales-summary-value">${clientTransferredNotCheckedAmount.toFixed(2)} ريال</div>
        <div class="order-sales-summary-meta">عددها: ${clientTransferredNotCheckedCount}</div>
      </div>

      <div class="order-sales-summary-card tone-purple order-sales-summary-ribbon">
        <div class="order-sales-summary-label">🎀 مبالغ الشريط — عماد</div>
        <div class="order-sales-summary-value">${phraseStripAmount.toFixed(2)} ريال</div>
        <div class="order-sales-summary-meta">العدد: ${phraseStripQuantity}</div>
      </div>
    </div>
  </section>
`;
  ...
  function switchTab(tabName, sourceButton) {
    ...
    document.querySelectorAll('.order-view-tab-btn').forEach(btn => {
...
<div class="delivery-time">
  <span class="delivery-time-label">موعد التسليم</span>
  <strong>${deliveryDate.toLocaleDateString('ar-SA', { weekday:'long', year:'numeric', month:'long', day:'numeric' })}</strong>
  <span class="delivery-time-clock">${deliveryDate.toLocaleTimeString('ar-SA', { hour:'numeric', minute:'2-digit', hour12:true })}</span>
</div>
      </div>
...
function resetSummaryDay() {
  ...
  function displayUnpaidOrders(monthKey, unpaidOrders) {
    ...
    html += `
      <div class="orange-line"></div>
      <section class="order-sales-summary order-sales-summary-unpaid">
        <h3 class="order-sales-summary-title danger">ملخص المبالغ غير المحولة</h3>
        <div class="order-sales-summary-grid order-sales-summary-grid-two">
          <div class="order-sales-summary-card tone-red centered">
            <div class="order-sales-summary-label">عدد الطلبات غير المحولة</div>
            <div class="order-sales-summary-value large">${unpaidOrders.length}</div>
          </div>
          <div class="order-sales-summary-card tone-red centered">
            <div class="order-sales-summary-label">إجمالي المبالغ غير المحولة</div>
            <div class="order-sales-summary-value large">${totalUnpaidAmount.toFixed(2)} ريال</div>
          </div>
        </div>
      </section>`;
  ...
  document.addEventListener("DOMContentLoaded", async () => {
    ...
    try {
      ...
      console.error("فشل تحميل بيانات الطلبات:", error);
      if (error && (error.status === 401 || error.message === "unauthorized")) return;
      alert("تعذر تحميل بيانات الطلبات الآن. لم يتم تعديل أي بيانات. تحقق من الاتصال ثم أعد المحاولة.");
      return;
    }
