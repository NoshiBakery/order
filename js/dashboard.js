...
(function(){
  ...
  function showPhoneReview(o){
    ...
    reviewIncoming.querySelector('b').textContent=incoming.formatted||o.phone;
    reviewSaved.textContent=pot.saved.formatted||pot.client.phone;
    const inDigits=incoming.normalized, savedDigits=pot.saved.normalized, diffs=[];
    for(let i=0;i<Math.min(inDigits.length,savedDigits.length);i++){
      if(inDigits[i]!==savedDigits[i]) diffs.push(`الخانة ${i+1}: ${inDigits[i]} بدل ${savedDigits[i]}`);
    }
    reviewDiff.textContent=`الاختلاف في ${pot.diff===1?'رقم واحد':'رقمين'} — ${diffs.join('، ')}`;
    reviewEditPhone.value=incoming.formatted||o.phone;
    reviewEditWrap.classList.add('hidden');
    reviewMsg.textContent='';
    reviewSameBtn.dataset.clientId=String(pot.client.id);
    openSheet('phoneReviewSheet');
  }
  reviewIncoming.addEventListener('click',()=>{
    reviewEditWrap.classList.toggle('hidden');
    if(!reviewEditWrap.classList.contains('hidden')){reviewEditPhone.focus();reviewEditPhone.select();}
  });
  ...
  reviewNewBtn.addEventListener('click',()=>{
    if(!selected)return;
    // لا نعتبرها عميلة جديدة بمجرد فتح نموذج الإنشاء.
    // إذا أُغلق النموذج بدون حفظ، يبقى الطلب "محتمل" ويظهر التحقق مرة أخرى.
