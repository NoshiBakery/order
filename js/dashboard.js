...
(function(){
  ...
  function showPhoneReview(o){
    ...
    const inDigits=incoming.normalized;
    const savedDigits=pot.saved.normalized;
    const diffs=[];
    for(let i=0;i<Math.min(inDigits.length,savedDigits.length);i++){
      if(inDigits[i]!==savedDigits[i]) diffs.push(`${inDigits[i]} بدل ${savedDigits[i]}`);
    }
    reviewDiff.textContent=`الاختلاف: ${diffs.join('، ')}`;
    reviewEditPhone.value=incoming.formatted||o.phone;
    reviewEditWrap.hidden=true;
    reviewMsg.textContent='';
    reviewSameBtn.dataset.clientId=String(pot.client.id);
    openSheet('phoneReviewSheet');
  }
  reviewIncoming.addEventListener('click',()=>{
    reviewEditWrap.hidden=!reviewEditWrap.hidden;
    if(!reviewEditWrap.hidden){reviewEditPhone.focus();reviewEditPhone.select();}
  });
