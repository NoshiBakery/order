/* =========================================================
   FILE: js/index.js
   SOURCE: inline script #1 from index.html
...
document.addEventListener("DOMContentLoaded", async () => {
  try {
    ...
    console.error("فشل تحميل بيانات الصفحة:", error);
    if (error && (error.status === 401 || error.message === "unauthorized")) return;
    alert("تعذر تحميل بيانات الصفحة الآن. لم يتم تعديل أي بيانات. تحقق من الاتصال ثم أعد المحاولة.");
    return;
  }
...
updateApproveButtonState();
    ...
    if (incomingDraft && Array.isArray(incomingDraft.items)) {
      const incomingBar=document.getElementById('incomingDraftBar');
      const incomingBarText=document.getElementById('incomingDraftBarText');
      if(incomingBar){
        incomingBar.hidden=false;
        if(incomingBarText) incomingBarText.textContent=`#${incomingDraft.externalOrderNumber||incomingDraft.externalRequestId||''} · ${incomingDraft.clientName||''}`;
      }
      const cancelIncomingBtn=document.getElementById('cancelIncomingDraftBtn');
      if(cancelIncomingBtn && !cancelIncomingBtn.dataset.bound){
        cancelIncomingBtn.dataset.bound='1';
        cancelIncomingBtn.addEventListener('click',async()=>{
          const yes=window.NoshiUI ? await NoshiUI.confirm('سيتم إلغاء تجهيز هذا الطلب والعودة لطلبات الموقع بدون قبول الطلب أو حفظه داخليًا.',{title:'رجوع بدون قبول',confirmText:'رجوع',cancelText:'متابعة التجهيز'}) : confirm('الرجوع بدون حفظ؟');
          if(!yes)return;
          try{await NoshiDB.remove('incomingOrderDraft');location.href='dashboard.html';}
          catch(error){console.error(error);window.NoshiUI?NoshiUI.alert('تعذر إلغاء التجهيز الآن. لم يتم تغيير حالة الطلب الخارجي.',{title:'تعذر الرجوع'}):alert('تعذر الرجوع');}
        });
      }
    ...
    if (generalDiscount > 0 || individualDiscounts.length > 0) {
      ...
      if (individualDiscounts.length > 0) {
        message += individualDiscounts.length === 1 ? 
          `🏷️ تم تطبيق خصم على المنتج التالي:\n` : 
          `🏷️ وتم تطبيق خصومات على المنتجات التالية:\n`;
...
/* =========================================================
   DELIVERY FEE DISTRIBUTION CONTRACT
   - No separate "smart distribution" button exists.
   - The total delivery amount already present in the cart is the budget.
   - New automatic slots share the remaining budget equally.
   - A fee edited by the user becomes manual and is never overwritten.
   - Legacy saved split fees without feeManual are preserved as manual.
========================================================= */
function rebalanceDeliveryFees(plan){
  if(!Array.isArray(plan)||!plan.length)return;
  const budgetCents=Math.max(0,Math.round(currentDeliveryFeeTotal()*100));
  const manual=plan.filter(s=>s.feeManual);
  const automatic=plan.filter(s=>!s.feeManual);
  if(!automatic.length)return;
  const manualCents=manual.reduce((sum,s)=>sum+Math.max(0,Math.round(Number(s.deliveryFee||0)*100)),0);
  const remaining=Math.max(0,budgetCents-manualCents);
...
function normalizePlanForCurrentCart(existing) {
  ...
  const plan = Array.isArray(existing) ? existing.map((slot, idx) => ({
    ...
    // Old multi-slot orders predate feeManual; preserve those saved fees.
    feeManual: Object.prototype.hasOwnProperty.call(slot,'feeManual') ? !!slot.feeManual : (Array.isArray(existing) && existing.length > 1),
...
function openSplitDeliveryPlanner() {
  ...
  style.textContent = `
    ...
    #splitDeliveryPlanner .sp-title{font-size:20px;font-weight:950;color:#241f2b}
    ...
    #splitDeliveryPlanner .sp-fields{display:grid;grid-template-columns:minmax(0,1.55fr) minmax(0,1fr) minmax(74px,.45fr);gap:7px;align-items:end}.sp-field{min-width:0;font-size:10px;font-weight:850;color:#615969}.sp-field input,.sp-field select{display:block;width:100%;min-width:0;height:34px;margin-top:4px;border:1px solid #ddd8e2;border-radius:9px;background:#fff;padding:3px 6px;font:700 10.5px -apple-system,BlinkMacSystemFont,'Segoe UI',Tahoma,Arial,sans-serif;color:#27232c;outline:none}.sp-field input[type=datetime-local]{font-size:9.5px;padding-inline:3px}.sp-field input:focus,.sp-field select:focus{border-color:#8b78e6;box-shadow:0 0 0 2px rgba(139,120,230,.10)}
    #splitDeliveryPlanner .sp-drop{min-height:108px;margin-top:9px;padding:8px;border:2px dashed #d8d1e8;border-radius:14px;background:#fbfaff;transition:.15s;cursor:pointer}.sp-drop.is-target{border-color:#6c5ce7;background:#f0edff}.sp-drop.has-selection{border-color:#6c5ce7;background:#f6f3ff}
    #splitDeliveryPlanner .sp-drop-label{font-size:11px;color:#6f6680;margin:0 2px 7px;font-weight:800}.sp-empty{display:grid;place-items:center;min-height:70px;color:#766a84;font-size:12px;font-weight:800}
    ...
    #splitDeliveryPlanner .sp-unit{position:relative;min-width:0;background:#fff;border:1px solid #e3dfe7;border-radius:13px;padding:6px;box-shadow:0 2px 8px rgba(0,0,0,.055);cursor:pointer;user-select:none;-webkit-user-select:none;-webkit-tap-highlight-color:transparent}.sp-unit.selected{border:2px solid #2f80ed;background:#eaf4ff;box-shadow:0 0 0 4px rgba(47,128,237,.16),0 4px 12px rgba(47,128,237,.13);transform:translateY(-1px)}.sp-unit.selected .sp-unit-name{color:#1559a6}.sp-unit img{display:block;width:100%;aspect-ratio:1/1;object-fit:cover;border-radius:9px;background:#f1f1f1;pointer-events:none}.sp-unit-name{font-size:11px;font-weight:900;color:#332c38;text-align:center;line-height:1.3;margin-top:6px;min-height:29px;display:flex;align-items:center;justify-content:center;pointer-events:none}
    ...
    @media(max-width:640px){#splitDeliveryPlanner{padding:0;align-items:flex-end}#splitDeliveryPlanner .sp-panel{width:100%;max-width:none;max-height:96dvh;border-radius:24px 24px 0 0;padding:12px 10px max(12px,env(safe-area-inset-bottom))}.sp-fields{grid-template-columns:minmax(0,1.35fr) minmax(82px,.65fr)!important;gap:6px!important}.sp-fields .sp-field:first-child{grid-column:1/-1}.sp-field{font-size:9.5px!important}.sp-field input,.sp-field select{height:32px!important;font-size:10px!important;padding:3px 5px!important}.sp-field input[type=datetime-local]{font-size:9px!important}.sp-products{grid-template-columns:repeat(3,minmax(0,1fr))!important}.sp-unit-name{font-size:10.5px!important}.sp-drop{min-height:102px!important}}
  `;
  ...
  panel.innerHTML = `
    <div class="sp-top">
      <div><div class="sp-title">تقسيم التسليم</div></div>
  ...
  function makeUnitCard(key) {
    ...
    card.className = 'sp-unit' + (selectedUnitKey === key ? ' selected' : '');
    card.dataset.unit = key;
    card.setAttribute('role', 'button');
    card.setAttribute('tabindex', '0');
    card.setAttribute('aria-pressed', selectedUnitKey === key ? 'true' : 'false');
    card.setAttribute('aria-label', `اختيار ${u.label}`);
    ...
    const choose = () => {
      selectedUnitKey = selectedUnitKey === key ? null : key;
      render();
    };
    ...
    working.forEach((slot, idx) => {
      ...
      if (working.length > 1) {
        ...
        remove.addEventListener('click', () => {
          ...
          if (selectedUnitKey && (removed.unitKeys || []).includes(selectedUnitKey)) selectedUnitKey = null;
          rebalanceDeliveryFees(working);
          render();
        });
      ...
      feeInput.addEventListener('input', () => { slot.deliveryFee = Math.max(0, Number(feeInput.value || 0)); });
      feeInput.addEventListener('change', () => {
        slot.deliveryFee = Math.max(0, Number(feeInput.value || 0));
        slot.feeManual = true;
        rebalanceDeliveryFees(working);
        render();
      });
      ...
      label.textContent = 'اسحب منتجًا هنا';
      ...
      if (!productGrid.children.length) {
        ...
        empty.textContent = selectedUnitKey ? 'اضغط هنا لنقل المنتج المحدد' : 'اختر منتجًا أولًا';
        drop.appendChild(empty);
      } else {
        drop.appendChild(productGrid);
      }
      if (selectedUnitKey) drop.classList.add('has-selection');
      drop.addEventListener('click', e => {
        if (e.target.closest('.sp-unit')) return;
        if (!selectedUnitKey) return;
        const movingKey = selectedUnitKey;
        selectedUnitKey = null;
        moveUnit(movingKey, slot.id);
      });
  ...
  panel.querySelector('#plannerSave').addEventListener('click', async () => {
    ...
    const expectedFee = Math.round(currentDeliveryFeeTotal() * 100);
    const splitFee = working.reduce((sum, slot) => sum + Math.max(0, Math.round(Number(slot.deliveryFee || 0) * 100)), 0);
    if (expectedFee !== splitFee) {
      await NoshiUI.alert(`مجموع التوصيل في التقسيم (${(splitFee/100).toFixed(2)} ر.س) يجب أن يساوي إجمالي التوصيل الموجود في السلة (${(expectedFee/100).toFixed(2)} ر.س). عدّل القيم ثم احفظ.`, { title: 'مراجعة رسوم التوصيل' });
      return;
    }
...
function syncThemeSettingButton(){
  if(!themeSettingBtn || !window.NoshiTheme) return;
  const dark = NoshiTheme.current() === 'dark';
  themeSettingBtn.textContent = dark ? '☀️ تفعيل النمط النهاري' : '🌙 تفعيل النمط الليلي';
}
if(themeSettingBtn){
  themeSettingBtn.addEventListener('click',()=>{ if(window.NoshiTheme) NoshiTheme.toggle(); syncThemeSettingButton(); });
  window.addEventListener('noshi:themechange',syncThemeSettingButton);
  syncThemeSettingButton();
}
