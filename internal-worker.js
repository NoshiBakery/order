...
function validDataKey(key) {
  ...
  // Monthly sales partitions are safe dynamic keys, e.g. salesData_2026_09.
  // The legacy salesData key stays supported during the compatibility period.
  return CLOUD_DATA_KEYS.has(value) || /^salesData_\d{4}_\d{2}$/.test(value);
}
