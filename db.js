...
(function () {
  ...
  // UI preferences are device-local by design. They must never block
  // business-data loading/saving or cause cloud revision conflicts.
  ...
  function isCloudKey(key) {
    ...
    return cloudEnabled() && !LOCAL_PREFERENCE_KEYS.has(value) && (CLOUD_KEYS.includes(value) || isSalesMonthStorageKey(value));
  }
  ...
  function openDB() {
    ...
    dbPromise = new Promise((resolve, reject) => {
      ...
      const timeoutId = setTimeout(() => {
        ...
        settled = true;
        dbPromise = null;
        reject(new Error("تأخر فتح التخزين المحلي. أغلق الصفحة وافتحها من جديد."));
      }, 8000);
      ...
      request.onsuccess = event => {
        if (settled) { try { event.target.result.close(); } catch (_) {} return; }
        settled = true; clearTimeout(timeoutId);
        const db = event.target.result;
        db.onversionchange = () => { try { db.close(); } catch (_) {} dbPromise = null; };
        resolve(db);
      };
      request.onerror = () => { if(settled)return; settled=true; clearTimeout(timeoutId); dbPromise=null; reject(request.error || new Error("فشل فتح التخزين المحلي")); };
      request.onblocked = () => { if(settled)return; settled=true; clearTimeout(timeoutId); dbPromise=null; reject(new Error("التخزين المحلي محجوب. أغلق الصفحات الأخرى ثم حاول مرة أخرى.")); };
    });
    return dbPromise;
  }
  ...
  async function readWithRetry(operation) {
    ...
    for (let i = 0; i < delays.length; i++) {
      ...
      try { return await operation(); }
        ...
        lastError = error;
        if (!shouldRetryRead(error) || i === delays.length - 1) throw error;
      }
    }
    throw lastError;
  }
  ...
  async function cloudGet(key, defaultValue = null) {
    const result = await readWithRetry(() => cloudApi(`/api/data/${encodeURIComponent(key)}`));
  ...
  async function cloudGetMany(keys, defaults = {}, options = {}) {
    ...
    const result = await readWithRetry(() => cloudApi(`/api/data?keys=${encodeURIComponent(uniqueKeys.join(","))}`));
    ...
    const [cloudValues, localPairs] = await Promise.all([
      ...
      Promise.all(localKeysWanted.map(async key => {
        ...
        try {
          ...
          if (LOCAL_PREFERENCE_KEYS.has(key)) {
            console.warn(`تعذر قراءة تفضيل الواجهة ${key}; تم استخدام القيمة الافتراضية.`, error);
            return [key, fallback];
          }
          throw error;
        }
  ...
  async function keys() {
    ...
    const local = (await localKeys()).filter(k => LOCAL_ONLY_KEYS.has(k) || LOCAL_PREFERENCE_KEYS.has(k));
  ...
  async function getAll() {
    ...
    [...LOCAL_ONLY_KEYS, ...LOCAL_PREFERENCE_KEYS].forEach(key => {
