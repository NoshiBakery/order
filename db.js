...
(function () {
  ...
  async function cloudGetMany(keys, defaults = {}, options = {}) {
    const uniqueKeys = [...new Set(keys.map(k => String(k || "")).filter(key => CLOUD_KEYS.includes(key)))];
  ...
  async function getAll() {
    ...
    // قراءة لايف واحدة لجميع مفاتيح السحابة بدل status + طلب HTTP منفصل لكل مفتاح.
    // لا يوجد cache هنا: كل استدعاء يرجع آخر نسخة مؤكدة من الـWorker/D1.
    const [output, local] = await Promise.all([
      cloudGetMany(CLOUD_KEYS, {}, { includeMissing: false }),
      localGetAll()
    ]);
