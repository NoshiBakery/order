...
- All user-facing times MUST use the 12-hour clock with Arabic `ص / م`.
- Stored date/time strings are not converted; only display formatting changes.
- When a delivery date is shown, prefer `YYYY-MM-DD (weekday) · h:mm ص/م`.

## Incoming website order cancel contract
- Clicking Accept on the dashboard only creates the local `incomingOrderDraft` and navigates to Products.
- The external order MUST remain `new` until the internal sales order is saved successfully.
- The Products page may safely cancel preparation by deleting only `incomingOrderDraft` and returning to the dashboard.
- Never mark the external order accepted before the internal save succeeds.
